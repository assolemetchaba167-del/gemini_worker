  ...
  async fetch(r, env) {
    ...
    if (body.type === "text") {
      try {
        ...
        return new Response(JSON.stringify({error: "Erreur IA: "+e.message}), {status:500, headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}});
      }
    }
    return new Response(JSON.stringify({error:"Type non supporté"}), {status:400, headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}});
  }
};
